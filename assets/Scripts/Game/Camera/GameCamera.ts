...
export class GameCamera extends Component {
  public followTargetNode: Node = null!; //相机跟随的目标节点
  ...
  lateUpdate() {
    if (!this.followTargetNode || !this.followTargetNode.worldPosition) {
      return;
    }

    this._targetCameraWorPos = this._targetCameraWorPos.lerp(this.followTargetNode.worldPosition, 0.5);
