import { _decorator, Component, UITransformComponent, SpriteComponent, tween, clamp, UITransform } from "cc";
...
export class BossBloodBar extends Component {
  ...
  onEnable() {
    EventManager.Instance.on(Constant.EVENT_TYPE.REFRESH_BOSS_BLOOD, this._refreshBossBlood, this);
  }

  onDisable() {
    EventManager.Instance.off(Constant.EVENT_TYPE.REFRESH_BOSS_BLOOD, this._refreshBossBlood, this);
  }

  protected onLoad(): void {
    this.UIComWhiteBar = this.getComponent(UITransform);
    this.UIComRedBar = this.node.getChildByName("redBar").getComponent(UITransform);
  }
        ...
        tween(this.UIComWhiteBar)
          ...
          .call(() => {
            if (ratio <= 0) {
              ...
              GameApp.Instance.boss.isDie = true;
            }
          })
          .start();
      }
